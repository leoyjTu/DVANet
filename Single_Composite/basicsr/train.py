...
def init_loggers(opt):
    ...
    if opt['logger'].get('use_tb_logger') and 'debug' not in opt['name']:
        tb_logger = init_tb_logger(
            log_dir=osp.join('experiments', opt['name'], 'tb_logs'))
...
def format_metric_name(metric_name):
    """Convert metric keys like m_psnr -> PSNR, m_ssim -> SSIM."""
    if metric_name.startswith('m_'):
        metric_name = metric_name[2:]
    return metric_name.upper()


def save_metrics_to_txt(opt, metrics_dict, current_iter):
    """Append validation metrics to a single txt file."""
    timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
    metrics_dir = osp.join('experiments', opt['name'], 'metrics')
    os.makedirs(metrics_dir, exist_ok=True)

    filename = osp.join(metrics_dir, 'validation_metrics.txt')

    with open(filename, 'a', encoding='utf-8') as f:
        f.write("=" * 60 + "\n")
        f.write("DVANet Validation Metrics Report\n")
        ...
        for metric_name, value in metrics_dict.items():
            ...
            if isinstance(value, (int, float)):
                f.write(f"{display_name}: {value:.6f}\n")
            else:
...
def main():
    ...
    if resume_state is None:
        make_exp_dirs(opt)
        if opt['logger'].get('use_tb_logger') and \
           'debug' not in opt['name'] and \
           opt['rank'] == 0:
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            if opt.get('val') is not None and (
                ...
                # save validation metrics into one txt file
                if hasattr(model, 'log_dict') and model.log_dict:
                    metric_log = {
                        k: v for k, v in model.log_dict.items()
                        if k.startswith('m_')
                    }
                    if metric_log:
                        metrics_file = save_metrics_to_txt(opt, metric_log, current_iter)
